...
void OrdenaNome (Aluno lista[], int tam){
	int troca = 1, c;
	Aluno aux;
	...
	while (troca == 1){
		troca = 0;
		for (c = 0; c < tam - 1; c++){
			if (strcmp(lista[c].nome, lista[c + 1].nome) > 0){
				aux = lista[c];
				lista[c] = lista[c+1];
				lista[c+1] = aux;
				troca = 1;
...
void OrdenaMedia (Aluno lista[], int tam){
	int troca = 1, c;
	Aluno aux;
	float media1, media2;
	...
	while (troca == 1){
		troca = 0;
		for (c = 0; c < tam - 1; c++){
			media1 = CalculaMedia(lista[c].nota1, lista[c].nota2);
			media2 = CalculaMedia(lista[c + 1].nota1, lista[c + 1].nota2);
			if (media1 < media2){
				aux = lista[c];
				lista[c] = lista[c+1];
				lista[c+1] = aux;
...
int main (){
	...
	OrdenaNome (aluno, tam);
	ImprimelistaAlunos(aluno, tam);

	OrdenaMedia (aluno, tam);
	ImprimelistaAlunos(aluno, tam);
